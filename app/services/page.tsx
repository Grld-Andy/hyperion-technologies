    ...
    <>
      <section className="bg-primary-deep py-20 text-primary-foreground">
        <div className="container-hyperion text-center">
          <span className="text-xs font-semibold uppercase tracking-[0.2em] text-gold-soft">Services</span>
        ...
        <div className="container-hyperion grid grid-cols-1 gap-6 md:grid-cols-2">
          {services.map((s) => (
            <div
              key={s.slug}
              id={s.slug}
              className="rounded-2xl border border-border p-8 transition-all hover:border-gold-soft hover:shadow-lg"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary text-gold-soft">
                <s.icon className="h-6 w-6" />
              </div>
              <h2 className="mt-5 font-display text-xl font-semibold text-primary-deep">{s.name}</h2>
              <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{s.description}</p>
              <ul className="mt-5 space-y-2">
                {s.highlights.map((h) => (
                  <li key={h} className="flex items-start gap-2 text-sm text-primary-deep">
      ...
      <section className="bg-primary-deep py-16 text-center text-primary-foreground">
        <div className="container-hyperion">
          ...
          <p className="mt-3 text-primary-foreground/75">Reach out and tell us what you need — we&apos;ll let you know how we can help.</p>
